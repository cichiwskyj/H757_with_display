...
#include "is42s32800j/is42s32800j.h"
...
// # Defined by me
// #############################
#define LCDBUFFER ((void*)0xD0000000)
...
// # Defined by me
// #############################
int32_t Get_Tick_Wrapper(void);
int32_t DSI_IO_Write_Wrapper(uint16_t ChannelNbr, uint16_t Reg, uint8_t* pData, uint16_t Size);
int32_t DSI_IO_Read_Wrapper(uint16_t ChannelNbr, uint16_t Reg, uint8_t* pData, uint16_t Size);
HAL_StatusTypeDef Driver_SDRAM_BANK2_Init(SDRAM_HandleTypeDef* hSdram);
int32_t RegisterDefaultMspCallbacks_LTDC_And_DSI();
void Driver_LCD_display_reset(void);
static void callback_ltdc_msp_init(LTDC_HandleTypeDef* hltdc);
static void callback_ltdc_msp_deinit(LTDC_HandleTypeDef* hltdc);
...
static void Driver_SDRAM_IS42S32800J_MspInit(SDRAM_HandleTypeDef* hSdram) {
	...
	/* GPIOG configuration */
	gpio_init_structure.Pin = GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_2 | GPIO_PIN_3 | GPIO_PIN_4 | GPIO_PIN_5 | GPIO_PIN_8 | GPIO_PIN_15;
	...
	/* GPIOI configuration */
	gpio_init_structure.Pin = GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_2 | GPIO_PIN_3 | GPIO_PIN_4 | GPIO_PIN_5 | GPIO_PIN_6 | GPIO_PIN_7 | GPIO_PIN_9 | GPIO_PIN_10;
	...
	HAL_NVIC_SetPriority(SDRAM_MDMAx_IRQn, SDRAM_IT_PRIORITY, 0);
	HAL_NVIC_EnableIRQ(SDRAM_MDMAx_IRQn);
}

int32_t Driver_SDRAM_IS42S32800J_Init(uint32_t Instance) {
	int32_t ret;
	...
	if (Instance >= SDRAM_INSTANCES_NBR) {
		...
		Driver_SDRAM_IS42S32800J_MspInit(&hsdram1);

		if (ret == 0) {
			if (Driver_SDRAM_BANK2_Init(&hsdram1) != HAL_OK) {
				ret = -1;
			} else {
...
int32_t RegisterDefaultMspCallbacks_LTDC_And_DSI() {
	if (HAL_LTDC_RegisterCallback(&hltdc, HAL_LTDC_MSPINIT_CB_ID, callback_ltdc_msp_init) != HAL_OK) {
		return -4;
	} else {
		if (HAL_LTDC_RegisterCallback(&hltdc, HAL_LTDC_MSPDEINIT_CB_ID, callback_ltdc_msp_deinit) != HAL_OK) {
			return -4;
		}
	}
...
void Driver_LCD_display_reset(void) {
	...
	HAL_GPIO_WritePin(GPIO_DSI_RST_GPIO_Port, GPIO_DSI_RST_Pin, GPIO_PIN_SET); /* Deactivate XRES */
...
/* USER CODE BEGIN Boot_Mode_Sequence_2 */
  ...
  Driver_SDRAM_IS42S32800J_Init(0);
	/* USER CODE END 2 */
	...
	/* add events, ... */
	OTM8009A_DisplayOff(Lcd_CompObj);
	for (size_t i = 0; i < 0x000FFFFF; ++i) { // Just some busywork wait
		__NOP();
	}
	// HAL_Delay(1000); //TODO: Why is this not working
	OTM8009A_DisplayOn(Lcd_CompObj);
	{
		size_t total = 800 * 480 * 4;
		memset(LCDBUFFER, 0x00, total * 2);
		// ARGB8888 color 0xFFFFAA00, which looks sort of orange
		for (size_t i = 0; i < total; i = i + 4) {
			*(uint8_t*)(LCDBUFFER + i) = 0xFF;
			*(uint8_t*)(LCDBUFFER + i + 1) = 0xFF;
			*(uint8_t*)(LCDBUFFER + i + 2) = 0xAA;
			*(uint8_t*)(LCDBUFFER + i + 3) = 0x00;
		}
	}
...
static void MX_DSIHOST_DSI_Init(void) {
	...
	// static void* Lcd_CompObj = NULL;
	IOCtx.Address = 0;
	IOCtx.GetTick = Get_Tick_Wrapper;
	IOCtx.WriteReg = DSI_IO_Write_Wrapper;
	IOCtx.ReadReg = DSI_IO_Read_Wrapper;
	int32_t res = 0;
	if ((res = OTM8009A_RegisterBusIO(&OTM8009AObj, &IOCtx)) != OTM8009A_OK) {
		while (1) {
			__NOP();
			// TODO: Some error occured that we can handle at some point
		}
	}
	...
	OTM8009A_SetBrightness(Lcd_CompObj, 100);
	OTM8009A_DisplayOn(Lcd_CompObj);
...
static void MX_LTDC_Init(void) {
	...
	RegisterDefaultMspCallbacks_LTDC_And_DSI();
